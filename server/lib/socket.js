...
io.on('connection', async socket => {
	...
	socket.on("result_executeTestSuite", async (message)=>{
		...
		const notifySocMap = socketMapNotify[clientName];
			if (!status && execReq) { // This block is for report data
				...
				try {
					...
					if (Object.keys(reportData.overallstatus).length !== 0) {
						...
						const reportid = await executor.insertReport(executionid, scenarioid, browserType, userInfo, reportData);
			...
			} else if (execReq) { // This block will trigger when resultData.status has "success or "Terminate"
				try {
					let result = status;
					let report_result = {};
					...
					if (notifySocMap[invokinguser] && notifySocMap[invokinguser].connected) { // This block is only for active mode
