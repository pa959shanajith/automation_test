...
module.exports = {
    init: function (_runtime) {
        log = _runtime.log;
        storageModule = _runtime.storage;
        settings = _runtime.settings;
    },
    generatePayload: function (event, data) {
        if (event === 'report') {
            return reportStatus(data);
        } else if (event === 'user-update') {
            return userUpdate(data);
        } else if (event === 'schedule-state') {
            return scheduleState(data);
        } else if (event === 'asset-state') {
            return assetAllocation(data);
        } else if (event === 'user-project-state') {
            return projectAllocation(data);
        }
        return when.reject({error:{ msg: event + " :: Notification event not found", code: ""}});
    }
}
