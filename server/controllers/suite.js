			...
			if (event == SOCK_NA) {
				...
				if (status == "success") {
					...
					if (resSent && notifySocMap[username] && notifySocMap[username].connected) {
						...
						queue.Execution_Queue.add_pending_notification("",execStatus,username);
						rsv(DO_NOT_PROCESS);
					} else rsv(execStatus);
				}
				...
				if (!status) { // This block is for report data
					...
					try {
						...
						if (resSent && notifySocMap[username] && notifySocMap[username].connected) { // This block is only for active mode
							...
							queue.Execution_Queue.add_pending_notification("",result,username);
							rsv(DO_NOT_PROCESS);
						}else {
							rsv(result);
						}
...
exports.ExecuteTestSuite_ICE = async (req, res) => {
	...
	const userInfo = { "userid": profile._id, "username": profile.name, "role": profile.role, "icename": "ice 1","invokingUser":req.session.userid,"invokingUserName":req.session.username};
