...
const CycleNode = props => {
    ...
    const [testSutie,setTestSutie] = useState([]);
    ...
    const handleClick = useCallback(async ()=>{
        ...
        if (collapse) {
            if (props.viewMappedFlies === "ZephyrUpdate" && props.section === "right"){
                ...
                dispatch({type: actionTypes.SHOW_OVERLAY, payload: 'Loading Test Suites...'});
            }
            ...
            if(props.viewMappedFlies === "ZephyrUpdate" && props.section !== "right") {
                ...
                let apiObj = Object.assign({"action": 'azureTestSuites'},
                props.azureLogin,
                props.selectedProject,
                {'testplandetails': props.testPlansDetails});
                data = await api.connectAzure_ICE(apiObj);
            ...
            if (data.error)
                ...
                if(props.section !== "right") {
                    setPhaseList(data.testsuites.filter((e) => !e.parentSuite));
                    setTestSutie(data.testsuites);
                }
                setCollapse(false);
            }
            dispatch({type: actionTypes.SHOW_OVERLAY, payload: ''});
                ...
                ? <div> {
                    phaseList
                        .map((phase, idx) => <PhaseNode 
                                                ...
                                                itemType={props.releaseId === 'Story' ? 'UserStory' : 'TestSuite'}
                                                testSuiteData = {testSutie}
                                            />)
                } </div>
                : null
            }
...
const PhaseNode = props => {
    ...
    let phasename = props.phase.name;
    let cyclephaseid = '';
    let parent = '';
    ...
    const handleClick = useCallback(async()=>{
        if (collapse) {
            if (props.viewMappedFlies === "ZephyrUpdate" && props.section === "right"){
                ...
                dispatch({type: actionTypes.SHOW_OVERLAY, payload: 'Loading Test Cases...'});
            }
            ...
            if(props.viewMappedFlies === "ZephyrUpdate" && props.section !== "right") {
                ...
                if(props.phase.hasChildren) {
                    setModules(props.testSuiteData.filter((e) => e.parentSuite?.id == props.phase?.id));
                }
                else{
                    let apiObj = Object.assign({"action": 'azureTestCases'},
                    props.azureLogin,
                    props.selectedProject,
                    {'testsuitedetails': props.phase},
                    {'testplandetails': props.testPlansDetails});
                    data = await api.connectAzure_ICE(apiObj);
                    dispatch({type: actionTypes.SHOW_OVERLAY, payload: ''});
                }
            ...
            if (data.error)
                ...
                if(props.section !== "right") {
                    // if(data.testcases) {
                    //     data.testcases = [props.phase,...data.testcases];
                    // }
                    setTestCases(data.testcases?data.testcases:[]);
                }
                // setModules(data.modules?data.modules:[]);
                setCollapse(false);
            }
            dispatch({type: actionTypes.SHOW_OVERLAY, payload: ''});
        }
                    ...
                    ? <div> {
                        modules
                            .map((phase, idx) => <PhaseNode 
                                                    ...
                                                    phaseDets={props.phaseDets}
                                                    setPhaseDets={props.setPhaseDets}
                                                    testSuiteData = {props.testSuiteData}
                                                    azureLogin = {props.azureLogin}
                                                    testPlansDetails = {props.testPlansDetails}
                                                    selectedProject = {props.selectedProject}
                                                />)
                    ...
                    ? <div> {
                        testCases
                            .map(testCase => <TestCaseNode 
                                                ...
                                                projectId={props.projectId}
                                                projectName={props.selectedProject.projectdetails.name}
                                                releaseId={props.releaseId}
                                                cycleid={props.cycleid}
                                                section={props.section}
                                                setRootCheck={props.setRootCheck}
...
const TestCaseNode = props => {
    ...
    const handleClick = e => {
        ...
        if (!e.ctrlKey) {
            ...
            newSelectedTCDetails.itemType = props.itemType;
            ...
            const mappedPair=[
                {
                    ...
                    projectName: selectedZTCDetails.selectedProjectName,		
