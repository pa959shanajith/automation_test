...
const ExecMetricsApi = props => {
    ...
    return ( <>
            ...
            <input data-test="lob-test" className={classes["execM__input"] + " " + (error.LOB ? classes["execM__inputError"]:"")} placeholder="Enter LOB" value={lob} onChange={(e)=>setLob(e.target.value.trim())} />

            <span className={classes["execM__inputLabel"]} >Status</span>
            <input data-test="status-test" className={classes["execM__input"]} placeholder="Enter Status" value={status} onChange={(e)=>setStatus(e.target.value.trim())} />

            <span className={classes["execM__inputLabel"]} >Execution ID</span>
            <input data-test="exec-test" className={classes["execM__input"]} placeholder="Enter Execution ID" value={executionId} onChange={(e)=>setExecutionId(e.target.value.trim())} />
    </> );
}
