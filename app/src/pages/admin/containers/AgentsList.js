...
const AgentsList = ({ setLoading, setShowConfirmPop, showMessageBar }) => {
    ...
    const onConfirmDeleteAgent = (name) => {
        ...
        showMessageBar(`${name} Agent Deleted. Please Save to reflect the changes`, 'SUCCESS');
    }
    const deleteAgent = (name) => {
        setShowConfirmPop({'title': 'Delete Avo Agent', 'content': <p>Are you sure, you want to delete <b>{name}</b> Agent?</p>, 'onClick': ()=>{ onConfirmDeleteAgent(name) }});
    }
    ...
    useEffect(()=> {
        (async()=>{
            ...
            if(agentList.error) {
                ...
                setOriginalAgentData(agentList.avoagents.map((agent) => ({
                    ...agent,
                    name: agent.Hostname,
                    state: 'idle'
                })));
                setAgentData(agentList.avoagents.map((agent) => ({
                    ...agent,
                    name: agent.Hostname,
                    state: 'idle'
                })));
    ...
    const handleAgentsSave = () => {
        (async() => {
            setLoading('Loading...');
            let requestData = [];
            for (let originalAgentIndex = 0; originalAgentIndex < originalAgentData.length; originalAgentIndex++) {
                if(agentData.some((agent) => agent.name === originalAgentData[originalAgentIndex].name)) {
                    for (let agentIndex = 0; agentIndex < agentData.length; agentIndex++) {
                        if(agentData[agentIndex].name === originalAgentData[originalAgentIndex].name && ((agentData[agentIndex].icecount !== originalAgentData[originalAgentIndex].icecount) || (agentData[agentIndex].status !== originalAgentData[originalAgentIndex].status))) {
    ...
    return (<>
        ...
        <div className="api-ut__btnGroup" style={{ display: 'flex', justifyContent: 'end' }}>
            ...
            <button data-test="submit-button-test" onClick={handleAgentsSave} disabled={!dataUpdated} >Save</button>
        </div>
        <div style={{ position: 'absolute', width: '96%', height: '76%' }}>
            <DetailsList columns={agentListHeader} items={
                ((searchText.length > 0) ? filteredList : agentData).map((agent, index) => ({
                    ...
                    status: <div className='detailslist_status'><p>Inactive</p> <Toggle onChange={() => onAgentToggle(agent.name)} checked={agent.status === 'active'} inlineLabel label="Active" /></div>,
                    deleteIcon: <img onClick={() => deleteAgent(agent.name)} src="static/imgs/DeleteIcon.svg" className="agents__action_icons" alt="Delete Icon"/>
                }))
