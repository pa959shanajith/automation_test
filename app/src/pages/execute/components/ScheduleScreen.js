...
import { schedulePeriod, scheduleWeeks } from "../../utility/mockData";
...
const ScheduleScreen = ({ cardData }) => {
  const [selectedSchedule, setSelectedSchedule] = useState(schedulePeriod[0]);
  ...
  const scheduleMonthly = [
    { name: <span>Day <InputText className="every_day" /> of every <InputText className="every_day" /> month(s).</span>, key: 'daymonth' },
    { name: <span className="day_week">The <AvoDropdown /> <AvoDropdown /> of every <InputText className="every_day" /> month(s).</span>,key: 'dayweek' },
  ];

  const [selectedDaily, setSelectedDaily] = useState(scheduleDaily[0]);
  const [selectedMonthly, setSelectedMonthly] = useState(scheduleMonthly[0]);
    ...
    let recurrenceObj = {
      ...
      WY: (
        <div className="col-12 lg:col-9 xl:col-9 md:col-8 sm:col-6 flex flex-wrap">
          <div style={{ height: '1rem' }}>Recur every <InputText /> week(s) on:</div>
          <div className="flex flex-wrap">
            {scheduleWeeks.map((el) => (
              <div key={el.key} style={{ paddingRight: '1rem' }}>
                <Checkbox
                  inputId={el.key}
                  name="daily"
                  value={el}
        ...
        children: [
          {
            ...
            label: (
              <div className="grid">
                ...
                <div className="col-12 lg:col-3 xl:col-3 md:col-4 sm:col-6" style={selectedSchedule.key !== "OT" ? { borderRight: '1px solid #000000' } : { border: 'none' }}>
  ...
  }, [selectedSchedule, selectedDaily, selectedMonthly]);
