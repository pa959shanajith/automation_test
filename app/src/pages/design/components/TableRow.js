            ...
            <div className="design__tc_row" onClick={!focused ? onRowClick : undefined}>
                <span className="objname_col">
                    ...
                    <div className="d__row_text" title={objName} >
                        <div style={{display:'contents'}}><span style={(props.testcaseDetailsAfterImpact && props.testcaseDetailsAfterImpact?.custNames?.includes(objName) && props.impactAnalysisDone?.addedTestStep)?{overflow: 'hidden',display: 'inline-block',width: '6rem',textOverflow: 'ellipsis'}:null}>{objName}</span>{objName !== "OBJECT_DELETED" && objName !== null && props.typesOfAppType === 'Web' && (!list.includes(objName)) && <span onMouseEnter={()=>showCard(objName)} className='pi pi-eye'></span>}</div>
