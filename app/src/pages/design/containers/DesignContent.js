...
import { UpperContent } from "../../design/components/ActionBarItems";
...
//   const UpperContent = ({setCheckedTc, setDTcFlag, isMac, setOverlay, disable, setShowDlg, dTcFlag, checkedTc, showDlg})
  ...
  const [isUnderReview, setIsUnderReview] = useState(
    props.current_task.status === "underReview"
  );
  ...
  const tableActionBtnGroup = [
    {
      title: "Add Test Step",
      img: "static/imgs/ic-jq-addstep.png",
      alt: "Add Steps",
      onClick: () => addRow(),
    },
    {
      title: "Edit Test Step",
      img: "static/imgs/ic-jq-editstep.png",
      alt: "Edit Steps",
      onClick: () => editRow(),
    },
    {
      title: "Select Test Step(s)",
      img: "static/imgs/ic-selmulti.png",
      alt: "Select Steps",
      onClick: () => selectMultiple(),
    },
    {
      title: "Drag & Drop Test Step",
      img: "static/imgs/ic-jq-dragstep.png",
      alt: "Drag Steps",
      onClick: () => toggleDrag(),
    },
    {
      title: "Copy Test Step",
      img: "static/imgs/ic-jq-copystep.png",
      alt: "Copy Steps",
      onClick: () => copySteps(),
    },
    {
      title: "Paste Test Step",
      img: "static/imgs/ic-jq-pastestep.png",
      alt: "Paste Steps",
      onClick: () => onPasteSteps(),
    },
    {
      title: "Skip Test Step",
      img: "static/imgs/ic-jq-commentstep.png",
      alt: "Comment Steps",
      onClick: () => commentRows(),
    },
  ];
...
// }, [current_task])
  ...
  useEffect(() => {
    if (props.imported) {
      fetchTestCases()
        .then((data) => {
          if (data === "success") setMsg(MSG.DESIGN.SUCC_TC_IMPORT);
          else setMsg(MSG.DESIGN.WARN_DELETED_TC_FOUND);
  ...
  useEffect(() => {
    if (
      ...
      fetchTestCases()
        .then((data) => {
          data !== "success" && setMsg(MSG.DESIGN.WARN_DELETED_TC_FOUND);
          setEdit(false);
          setStepSelect({ edit: false, check: [], highlight: [] });
          headerCheckRef.current.indeterminate = false;
          setDraggable(false);
...
// })},[])
          ...
          DesignApi.getScrapeDataScreenLevel_ICE(
            ...
            .then((scriptData) => {
              ...
              DesignApi.getKeywordDetails_ICE(appType)
                .then((keywordData) => {
                  if (keywordData === "Invalid Session")
                    return RedirectPage(history);
                    ...
                    for (let i = 0; i < testcase.length; i++) {
                      if ("comments" in testcase[i]) {
                        ...
                        if (appType === "Webservice") {
                          if (
                            testcase[i].keywordVal === "setHeader" ||
                            testcase[i].keywordVal === "setHeaderTemplate"
                          ) {
                            testcase[i].inputVal[0] = testcase[i].inputVal[0]
                              .split("##")
                              .join("\n");
                          }
                        }
                        ...
                        testcase[i].keywordVal =
                          testcase[i].keywordVal[0].toLowerCase() +
                          testcase[i].keywordVal.slice(1);
                  ...
                  setPastedTC([]);
                  setObjNameList(
                    getObjNameList(props.current_task.appType, scriptData.view)
                  );
    ...
    if (userInfo.role !== "Viewer") {
      if (reusedTC && !confirmed) {
        props.setShowConfirmPop({
          title: "Save Testcase",
          content: "Testcase has been reused. Are you sure you want to save?",
          onClick: () => {
            props.setShowConfirmPop(false);
            saveTestCases(null, true);
          },
        });
        return;
      }

      let { screenId, testCaseId, testCaseName, versionnumber } =
        props.current_task;
      let import_status = false;

      if (
        String(screenId) !== "undefined" &&
        String(testCaseId) !== "undefined"
      ) {
        let errorFlag = false;
        ...
        for (let i = 0; i < testCases.length; i++) {
          ...
          if (!testCases[i].custname || !testCases[i].keywordVal) {
            ...
            setMsg(
              MSG.CUSTOM(
                `Please select ${col} Name at Step No. ${step}`,
                VARIANT.WARNING
              )
            );
            errorFlag = true;
            ...
            if (
              testCases[i].keywordVal === "SwitchToFrame" &&
              String(testScriptData) !== "undefined"
            ) {
              ...
              for (let j = 0; j < scriptData.length; j++) {
                if (
                  ![
                    ...
                    "@Generic",
                    "@Custom",
                  ].includes(scriptData[j].custname) &&
                  scriptData[j].url !== ""
                ) {
            ...
            if (
              ["setHeader", "setHeaderTemplate"].includes(
                testCases[i].keywordVal
              )
            ) {
              if (typeof testCases[i].inputVal === "string")
                testCases[i].inputVal = testCases[i].inputVal.replace(
                  /[\n\r]/g,
                  "##"
                );
              else
                testCases[i].inputVal[0] = testCases[i].inputVal[0].replace(
                  /[\n\r]/g,
                  "##"
                );
            }
        ...
        if (!errorFlag) {
          DesignApi.updateTestCase_ICE(
            ...
            .then((data) => {
              ...
              if (data === "success") {
                if (
                  props.current_task.appType.toLowerCase() === "web" &&
                  Object.keys(modified).length !== 0
                ) {
                  let scrape_data = {};
                  let { appType, projectId, testCaseId, versionnumber } =
                    props.current_task;
                    ...
                    .then((res) => {
                      ...
                      for (let i = 0; i < scrape_data.view.length; i++) {
                        if (scrape_data.view[i].custname in modified) {
                          scrape_data.view[i].xpath =
                            modified[scrape_data.view[i].custname];
  ...
  const setRowData = (data) => {
    ...
    if (operation === "row") {
      ...
      if (
        ...
        testCases[rowIdx].inputVal[0] !== inputVal ||
  ...
  const submitTask = (submitOperation) => {
    ...
    reviewTask(
      ...
      taskname,
      recipients.groupids,
      recipients.additionalrecepients
    )
      .then((result) => {
        if (result === "fail") setMsg(MSG.GENERIC.WARN_NO_REVIEWER);
        else if (taskstatus === "reassign") {
          setMsg(MSG.DESIGN.SUCC_TASK_REASSIGN);
          redirectToPlugin();
        } else if (taskstatus === "underReview") {
          setMsg(MSG.DESIGN.SUCC_TASK_APPROVED);
          redirectToPlugin();
        } else {
          setMsg(MSG.DESIGN.SUCC_TASK_SUBMIT);
          redirectToPlugin();
        }
      })
      .catch((error) => {
  ...
  const deleteTestcase = () => {
    let testCases = [...testCaseData];
    if (testCases.length === 1 && !testCases[0].custname)
      setMsg(MSG.DESIGN.WARN_DELETE);
    else if (stepSelect.check.length <= 0) setMsg(MSG.DESIGN.WARN_SELECT_STEP);
    else if (reusedTC)
      props.setShowConfirmPop({
        title: "Delete Test Step",
        content: "Testcase has been reused. Are you sure you want to delete?",
        onClick: () => {
          props.setShowConfirmPop(false);
          onDeleteTestStep();
        },
  ...
  const selectSteps = (stepList) => {
    ...
    headerCheckRef.current.indeterminate =
      newChecks.length !== 0 && newChecks.length !== testCaseData.length;
    setShowSM(false);
  };
  ...
  const editRow = () => {
    ...
    let focus = [];
    runClickAway = false;
    if (check.length === 0 && highlight.length === 0)
      setMsg(MSG.DESIGN.WARN_SELECT_STEP_DEL);
    else {
      if (check.length === 1) focus = check;
      else if (highlight.length === 1 && !check.length) {
        focus = highlight;
        check = highlight;
      } else check = [];
      ...
      setDraggable(false);
      headerCheckRef.current.indeterminate =
        check.length !== 0 && check.length !== testCaseData.length;
    }
  };
  ...
  const copySteps = () => {
    ...
    if (selectedRows.length === 0) setMsg(MSG.DESIGN.WARN_SELECT_STEP_COPY);
      ...
      if (!copyErrorFlag) {
        copyContent = {
          appType: props.current_task.appType,
          testCaseId: props.current_task.testCaseId,
          testCases: copyTestCases,
        };
        dispatch({
          type: designActions.SET_COPYTESTCASES,
          payload: copyContent,
        });
        setEdit(false);
      }
  ...
  const pasteSteps = (stepList) => {
    ...
    let localPastedTc = [...pastedTC];
    copiedContent.testCases.forEach((testcase) =>
      testcase.objectid ? localPastedTc.push(testcase.objectid) : null
    );
  ...
  const commentRows = () => {
    ...
    let testCases = [...testCaseData];
    runClickAway = false;
    if (highlighted.length === 0 && selectedIndexes.length === 0)
      setMsg(MSG.DESIGN.WARN_SELECT_STEP_SKIP);
    else if (
      selectedIndexes.length === 1 &&
      !testCases[selectedIndexes[0]].custname
    )
      setMsg(MSG.DESIGN.WARN_EMP_STEP_COMMENT);
    else if (highlighted.length === 1 && !testCases[highlighted[0]].custname)
      setMsg(MSG.DESIGN.WARN_EMP_STEP_COMMENT);
    else {
      let toComment = [...new Set([...highlighted, ...selectedIndexes])];
      for (let idx of toComment) {
        if (stepSelect.edit && edit && stepSelect.highlight.includes(idx))
          continue;
        let testCase = { ...testCases[idx] };
        let isComment = testCase.outputVal.slice(-2) === "##";
        if (isComment)
          testCase.outputVal = testCase.outputVal.replace(/(;*##)$/g, "");
        else
          testCase.outputVal += testCase.outputVal.length === 0 ? "##" : ";##";
        ...
        <div>
          ...
          <p className="dc__checkbox-addRecp">
            <input
              id="dc__checkbox"
              onChange={() => {
                fetchSelectRecipientsData();
              }}
              type="checkbox"
              title="Notify Additional Users"
              className="checkAddUsers"
            />
            <span>Notify Additional Users</span>
          </p>
          <div className="dc__select-recpients">
            <div>
              <span className="leftControl" title="Token Name">
                Select Recipients
              </span>
              <SelectRecipients
                disabled={checkAddUsers()}
  ...
  const fetchSelectRecipientsData = async () => {
    ...
    if (!checkAddUsers) resetData();
      ...
      for (var i = 0; i < data.length; i++)
        if (data[i][3] !== "Admin")
          userOptions.push({ _id: data[i][1], name: data[i][0] });
  ...
  const getKeywords = useCallback(
    (objectName) =>
      getKeywordList(
        objectName,
        keywordList,
        props.current_task.appType,
        testScriptData
      ),
    [keywordList, props.current_task, testScriptData]
  );

  const getRowPlaceholders = useCallback(
    (obType, keywordName) => keywordList[obType][keywordName],
    [keywordList]
  );
    ...
    <>
      ...
      {showSM && (
        <SelectMultipleDialog
          data-test="d__selectMultiple"
          setShow={setShowSM}
          selectSteps={selectSteps}
          upperLimit={testCaseData.length}
        />
      )}
      {showPS && (
        <PasteStepDialog
      ...
      {showRemarkDlg && (
        <RemarkDialog
          remarks={testCaseData[parseInt(showRemarkDlg)].remarks}
          setShow={setShowRemarkDlg}
      ...
      {showDetailDlg && (
        <DetailsDialog
          TCDetails={testCaseData[showDetailDlg].addTestCaseDetailsInfo}
          setShow={setShowDetailDlg}
          onSetRowData={setRowData}
          idx={showDetailDlg}
        />
      )}
      ...
      {/* <Breadcrumb  className='breadcrumb-item'>
          ...
          <div className="d__btngroup">
            <div className="d__table_ac_btn_grp">
              {tableActionBtnGroup.map((btn, i) => (
                <button
                  data-test="d__tblActionBtns"
                  key={i}
                  className={
                    "d__tblBtn " +
                    (draggable && btn.alt && btn.alt === "Drag Steps"
                      ? "selected-btn"
                      : "")
                  }
                  onClick={() => btn.onClick()}
            ...
            <div className="d__taskBtns">
              <button
                className="d__taskBtn d__btn"
                data-test="d__saveBtn"
              ...
              <button
                className="d__taskBtn d__btn"
                data-test="d__deleteBtn"
                title="Delete Test Step"
                onClick={deleteTestcase}
                disabled={!stepSelect.check.length}
              >
                Delete
              </button>
           ...
           <Button label="Debug" className="p-button-warning" onClick={()=>debugTestCases(debugButton)}></Button>
            ...
            <div className="d__submit" data-test="d__actionBtn">
              {isUnderReview && (
                <>
                  <button
                    className="d__reassignBtn d__btn"
                    title="Reassign Task"
                  ...
                  <button
                    className="d__approveBtn d__btn"
                    title="Approve Task"
        ...
        <div className="d__table">
          <div className="d__table_header">
            ...
            <span className="sel_col d__sel_head">
              <input
                className="sel_obj"
                type="checkbox"
                checked={headerCheck}
                      ...
                      >
                        ...
                        >
                          <ReactSortable
                            filter=".sel_obj"
                            disabled={!draggable}
                            key={draggable.toString()}
                            list={testCaseData}
                            setList={setTestCaseData}
                            animation={200}
                            ghostClass="d__ghost_row"
                            onEnd={onDrop}
                          >
                            {testCaseData.map((testCase, i) => (
                              <TableRow
                                data-test="d__tc_row"
                                draggable={draggable}
                                key={i}
