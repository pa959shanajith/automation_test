        ...
        return (
            ...
            {(rowData)?<div>
                {(rowData.name === rowExpandedName.name)?<div className='btn__grp' style={{ cursor: assignUser ? 'pointer' : 'not-allowed' }}>
                    <img className='add' src='static/imgs/ic-jq-addsteps.png' alt='addrow' style={{marginTop:'0.5rem',width:'26px', height:'26px',opacity:assignUser?'1':'0.5'}}  onClick={assignUser? ()=>addRow():null} />
                    <Tooltip target=".add " position="bottom" content="  Add Test Step"/>
                    <img src='static/imgs/ic-jq-editsteps.png' alt='edit' className='edit' style={{width:'20px', height:'20px', marginTop:'0.7rem',opacity:assignUser?'1':'0.5'}} onClick={assignUser ?()=>editRow():null}/>
                    <Tooltip target=".edit " position="bottom" content="  Edit Test Step"/>
                    <img className='trash' src='static/imgs/ic-jq-deletesteps.png' alt='delete' style={{marginTop:'0.5rem', width:'26px', height:'26px' ,opacity:assignUser?'1':'0.5'}} title='Delete' onClick={assignUser ? ()=>deleteTestcase():null} />
                    <Tooltip target=".trash " position="bottom" content="  Delete"/>
                    <Divider type="solid" layout="vertical" style={{padding: '0rem', margin:'0rem'}}/>

                    <i className='pi pi-check-square' style={{width:'20px', height:'20px', marginTop:'0.8rem',color: 'black',opacity:assignUser?'1':'0.5'}} onClick={assignUser ?()=>selectMultiple():null}/>
                    <Tooltip target='.pi-check-square' position='bottom' content='  Select Test Step(s)'/>
                    <img src='static/imgs/ic-jq-dragsteps.png' alt='Drag Steps' className='drag' style={{width:'20px', height:'20px', marginTop:'0.7rem',opacity:assignUser?'1':'0.5'}} onClick={assignUser ?()=>toggleDrag():null}/>
                    <Tooltip target='.drag' position='bottom' content='  Drag & Drop Test Step'/>
                    <img src='static/imgs/ic-jq-copysteps.png' alt='Copy Steps' className='copy' style={{width:'20px', height:'20px', marginTop:'0.7rem',opacity:assignUser?'1':'0.5'}} onClick={assignUser ?()=>copySteps():null}/>
                    <Tooltip target='.copy' position='bottom'content='  Copy Test Step'/>
                    <img src='static/imgs/ic-jq-pastesteps.png' alt='Paste steps' className='paste' style={{width:'20px', height:'20px', marginTop:'0.7rem',opacity:assignUser?'1':'0.5'}} onClick={assignUser ?()=>onPasteSteps():null}/>
                    <Tooltip target=".paste" position='bottom' content='  Paste Test Step'/>
                    <Divider type="solid" layout="vertical" style={{padding: '0rem', margin:'0rem'}}/>

                    <img src='static/imgs/skip-test-steps.png' alt='comment steps'className='comment' style={{width:'20px', height:'20px', marginTop:'0.7rem',opacity:assignUser?'1':'0.5'}} onClick={assignUser ?()=>commentRows():null}/>
                    <Tooltip target=".comment " position="bottom" content="  Skip Test Step"/>
                    <Divider type="solid" layout="vertical" style={{padding: '0rem', margin:'0rem'}}/>

                    <img src='static/imgs/import_new_18x18_icons.png' className='ImportSSSS' alt='import' style={{marginTop:'0.6rem', width:'20px', height:'20px',opacity:assignUser?'1':'0.5'}} onClick={assignUser ?()=>importTestCase():null} />
                    ...
                    <Button label="Debug" size='small'  disabled={rowData.testCases.length===0 || debugEnable ||!assignUser} className="debuggggg" onClick={()=>{DependentTestCaseDialogHideHandler(); setVisibleDependentTestCaseDialog(true)}} outlined></Button>
