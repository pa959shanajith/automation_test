...
// Pairwise Popup
		$scope.pairwiseGenerate = function()
		{
			...
			utilityService.Pairwise(obj)
				...
				function(error) {console.log("Error while traversing DesignController.js file addUITestscripts method!! \r\n "+(error.TableData));});
			};
	...
	$scope.ExportPairwise=function(){
		...
		if(objBrMajorVersion== "9"){
			if(objbrowserName == "Microsoft Internet Explorer"){
				window.navigator.msSaveOrOpenBlob(new Blob([responseData], {type:"text/csv;charset=utf-8"}), filename);
			}
		}else{
