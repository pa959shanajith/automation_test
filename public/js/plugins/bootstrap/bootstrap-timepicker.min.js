    ...
    d.prototype = {
        ...
        decrementHour: function() {
            ...
            if (this.showMeridian){
                if (1 === this.hour){
                	if((this.$element.parent("span").siblings().children("input.fc-datePicker").val() === ("0" + new Date().getDate()).slice(-2) +"-"+ ("0" + (new Date().getMonth()+1)).slice(-2) +"-"+ new Date().getFullYear()) && this.hour <= hours){
                		return false;
                	}
                	else this.hour--;                	
                }
                	...
                	if((this.$element.parent("span").siblings().children("input.fc-datePicker").val() === ("0" + new Date().getDate()).slice(-2) +"-"+ ("0" + (new Date().getMonth()+1)).slice(-2) +"-"+ new Date().getFullYear()) && this.hour <= hours){
                        ...
                        if (0 === this.hour) 
                        	return this.hour = 23, this.toggleMeridian();
                        this.hour--                		
                	}
                }
            }
                ...
                var getHr = this.hour;
                //var flg = false;
                //if(getHr > 12){ getHr = (getHr - 12); flg = true; } else { getHr = this.hour;}
                getHr > 12? getHr = (getHr - 12) : getHr = this.hour;
                var sltDate = this.$element.parent("span").siblings().children("input.fc-datePicker").val().split("-");
                var curdate = new Date();
                curdate.setHours(0,0,0,0);
                hours <= 12 ? getHr = this.hour : getHr = getHr
                if (1 === getHr){
                	if((new Date(sltDate[2],(sltDate[1]-1),sltDate[0]).toLocaleString() === curdate.toLocaleString()) && getHr <= hours ){
                		return false;
                	}
                	else this.hour--;                	
                }
        ...
        decrementMinute: function(a) {
            ...
            getHr > 12? getHr = (getHr - 12) : getHr = this.hour;
            hours <= 12 ? getHr = this.hour : getHr = getHr
        	if((this.$element.parent("span").siblings().children("input.fc-datePicker").val() === ("0" + new Date().getDate()).slice(-2) +"-"+ ("0" + (new Date().getMonth()+1)).slice(-2) +"-"+ new Date().getFullYear()) && getHr <= hours && this.minute <= new Date().getMinutes() ){
        		return false;
        	}
        	else{
                var b;
        ...
        incrementHour: function() {
            if (this.showMeridian) {
                ...
                var sltdDate = this.$element.parent("span").siblings().children("input.fc-datePicker").val().split("-");
                var curDate = new Date();
                curDate.setHours(0,0,0,0);
                if(this.hour === this.maxHours - 1 && new Date(sltdDate[2],(sltdDate[1]-1),sltdDate[0]).toLocaleString() == curDate.toLocaleString()){
                    return false;
                }
                else return this.hour === this.maxHours - 1 ? void(this.hour = 0) : void this.hour++
            }
        },
        ...
        toggleMeridian: function() {
	      	if((this.$element.parent("span").siblings().children("input.fc-datePicker").val() === ("0" + new Date().getDate()).slice(-2) +"-"+ ("0" + (new Date().getMonth()+1)).slice(-2) +"-"+ new Date().getFullYear())){
	      		return false;
	      	}
	      	else this.meridian = "AM" === this.meridian ? "PM" : "AM"
        },
        update: function(a) {
